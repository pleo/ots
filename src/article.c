...
OtsSentence* ots_new_sentence(void) {
...
void ots_free_sentence(OtsSentence* sen) {
  if (NULL != sen) {
...
OtsArticle* ots_new_article(void) {
  OtsArticle* Doc = g_new0(OtsArticle, 1);
  Doc->lineCount  = 0;
  Doc->title      = NULL;
...
void ots_append_word(OtsSentence* aLine, const char* aWord) {
  if ((NULL == aWord) || (0 == strlen(aWord)) || (NULL == aLine)) {
    return;
  }
  aLine->wc++;
